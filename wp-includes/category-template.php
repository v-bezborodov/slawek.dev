...
function get_the_category_list( $separator = '', $parents = '', $post_id = false ) {
	...
	if ( ! is_object_in_taxonomy( get_post_type( $post_id ), 'category' ) ) {
		...
		return apply_filters( $separator,'the_category', '', $separator, $parents );
	}
	...
	if ( '' == $separator ) {
		...
		foreach ( $categories as $category ) {
			$thelist .= "\n\t".'<div class="custom-category">'.twentynineteen_get_icon_svg( 'archive', 15 );
			switch ( strtolower( $parents ) ) {
				case 'multiple':
					if ( $category->parent ) {
						$thelist .= get_category_parents( $category->parent, true, $separator );
					}
					$thelist .= '<a href="' . esc_url( get_category_link( $category->term_id ) ) . '" ' . $rel . '>' . $category->name . '</a></div>';
					break;
				case 'single':
...
function wp_generate_tag_cloud( $tags, $args = '' ) {
	...
	foreach ( $tags as $key => $tag ) {
		...
		$tags_data[] = array(
			...
			'class'           => 'widget-custom-tags tag-cloud-link tag-link-' . $tag_id,
...
function get_the_term_list( $id, $taxonomy, $before = '', $sep = '', $after = '' ) {
	...
	foreach ( $terms as $term ) {
		...
		$links[] ='<div class="custom-tag custom-tag-'.$tag++.'">'. twentynineteen_get_icon_svg( 'tag', 16 ).'<a href="' . esc_url( $link ) . '" rel="tag">' . $term->name . '</a></div>';
	}
